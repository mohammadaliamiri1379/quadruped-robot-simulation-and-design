...
clear all
warning off
t_to = 1 ;   %meghdar zaman ke yek pa harekat kone
L1 = 1 ; L2 = 1 ; %toole pa
d = 1.5 ;%  arze robot
t = 0:0.01:t_to ;
len_t= length(t) ;
...
x_03 = 0.5*(0.48*t(len_t)^2-0.12*t(len_t)^3) ;
X3 = zeros(len_t , 6) ;
teta0 = X2(len_t,:) ;
X3(1,:) = teta0 ;
for k=2:len_t
    fun = @(x)root2d2(x,t(k) , 1.5 ,.5 , x_03) ;
    X3(k,:) = fsolve(fun ,teta0 ) ;
    teta0 = X3(k,:) ;
end
...
x0 = -0.03*0+0.48*t(k)^2-0.12*t(k)^3 ;
z0 = 1.1574*t(k)^3-2.3148*t(k)^4+1.1574*t(k)^5 ;
...
for k=1:len_t
    angles(:,:,1,k) = [X3(k,1) , X3(k,2) , X3(k,3)] ;
    angles(:,:,2,k) = [X3(k,4) , X3(k,5) , X3(k,6)] ;
end
